...
public class OpponentCardDisplay : MonoBehaviour
{
    ...
    // Cardback adjusting
    public bool OpponentcardBack;
    public static bool OpstaticCardBack;
    ...
    public bool OpponentbeInGraveyard;
    ...
    void Start()
    {
        ...
        numberOfCardInDeck = OpponentDeck.OpdeckSize;
        ...
        OpponentbeInGraveyard = false;
        canBeDestroyed = true;

        // Link game objects in this script to existed gameobjects in unity
        enemy               = GameObject.Find("PlayerHealth");
        enemyZone           = GameObject.Find("PlayerDropPanel");
        battleZone          = GameObject.Find("OpponentDropPanel");
    ...
    void Update()
    {
        ...
        if (this.tag == "Clone") // this.tag will be equal to "Clone" when this card is drawn from deck (clone a top deck card)
        {
            ...
            OpponentDeck.OpdeckSize -= 1;
            displayCard[0] = OpponentDeck.OpstaticDeck[numberOfCardInDeck];
            OpponentcardBack = false;
            this.tag = "Untagged";
        }

        // If cards are in player's hand -> don't display cardback
        Opponenthand = GameObject.Find("OpponentHand");
        if (this.transform.parent == Opponenthand.transform.parent) { OpponentcardBack = false; }
        OpstaticCardBack = OpponentcardBack;
        ...
        if (TurnSystem.currentEnemyMana >= cost && summoned == false && OpponentbeInGraveyard == false && TurnSystem.isOpponentTurn == true)
        {
        ...
        if (canBeSummon) { gameObject.GetComponent<OpponentDrag>().enabled = true; summonBoarder.SetActive(true); }
        else            { gameObject.GetComponent<OpponentDrag>().enabled = false; summonBoarder.SetActive(false); }
        ...
        if (type == "unit")
        {
            ...
            if ((this.transform.parent.name == "OpponentDropPanel" || this.transform.parent.name == "PlayerDropPanel") && ableChangeDisplay)
            {
            ...
            if (TurnSystem.isOpponentTurn == false && summoned)
            {
            ...
            if (TurnSystem.isOpponentTurn && summoningSickness == false && cantAttack == false && power > 0 && this.transform.parent == battleZone.transform) { canAttack = true; }
            else { canAttack = false; }

            // If this unit can attack and not been destroy yet, display the attack boarder.
            if (canAttack && OpponentbeInGraveyard == false) { attackBorder.SetActive(true); }
    ...
    public void TargetEnemy()
    {
        ...
        {
            if (child.GetComponent<OpponentCardDisplay>().isTaunt)
            {
                hasTaunt = true;
                break;
            }
    ...
    public void Attack()
    {
        if (canAttack && summoned)
        {
            ...
            else // If the target is enemy unit
            {
                ...
                {
                    if (child.GetComponent<CardDisplay>().isTarget)
                    {
                        child.GetComponent<CardDisplay>().hurted += power;
                        hurted += child.GetComponent<CardDisplay>().power;
                        cantAttack = true;
        ...
        if (stopDealDamage == false)
        {
            ...
            else // If the target is enemy unit
            {
                ...
                foreach (Transform child in enemyZone.transform)
                {
                    if (child.GetComponent<CardDisplay>().isTarget)
                    {
                        child.GetComponent<CardDisplay>().hurted += x;
                        stopDealDamage = true;
    ...
    public void DestroyCard()
    {
        if (canBeDestroyed)
        {
            this.transform.SetParent(Opponentgraveyard.transform);
            canBeDestroyed = false;
            summoned = false;
            OpponentbeInGraveyard = true;
        }
    }
}
